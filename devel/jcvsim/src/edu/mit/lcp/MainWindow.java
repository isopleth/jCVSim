...
import jcvsim.compartment21.TiltTestFrame;
...
public class MainWindow {
    ...
    private java.util.List<Patient> patientList;
    public ControlToolBar toolbar;
    private ViewMap viewMap;
    private TabWindow stripChartTabWindow;
    private TabWindow XYPlotTabWindow;
    public TiltTestFrame tiltTestFrame;
    private ImagePanel imagePanel;

    public MainWindow() {
        frame = new JFrame(new ProgramVersion().toString()
                + " -- Model: " + CVSim.getSimulationModel());
    }
    ...
    public void createAndShowGUI() {
        ...
        // Default strip chart
        PlotWindow stripChart = new PlotWindow(new TraceListModel(), PlotType.STRIPCHART);
        ...
        // Default XY plot
        PlotWindow parametric = new PlotWindow(new TraceListModel(), PARAMETRIC);
        ...
        tabbedPane.addTab("Logging", new LogPanel());
...
// 	optionsPanel = new OptionsPanel();
    ...
    boolean limitedPrecisionDisplayIsEnabled() {
        return toolbar.limitedPrecisionDisplayIsEnabled();
    }
    ...
    private class CreateStripChartWindowAction extends AbstractAction {
        ...
        public void actionPerformed(ActionEvent e) {
            ...
            PlotWindow plotWindow = new PlotWindow(new TraceListModel(), STRIPCHART);
    ...
    private class CreateXYPlotWindowAction extends AbstractAction {
        ...
        public void actionPerformed(ActionEvent e) {
            ...
            PlotWindow plotWindow = new PlotWindow(new TraceListModel(), PARAMETRIC);
    ...
    public void createPatients() {
        ...
        String pt1History = String.format("This patient is 40 years old, is well developed, "
                + "and presents with dyspnea and chest pain on exertion, and frequent "
                + "ventricular arrhythmias. He has had several unexplained syncopal attacks, "
                + "from which he has recovered without incident. Physical exam revealed a "
                + "prominent LV impulse, a palpable fourth heart sound, a biphasic carotid pulse "
                + "with a sharp rise time, and a systolic ejection murmur which increased in " + "intensity with isuprel.");
        ...
        String pt2History = String.format("This patient is a 24 year old woman "
                + "who presents with a long history of severe dyspnea and now requires "
                + "constant oxygen supplements.  She also has had a long history of "
                + "peripheral edema.  Physical exam reveals elevated jugular venous "
                + "pressure, a prominent cardiac impulse in the subxyphoid area, and a "
                + "loud second heart sound which was not split.  She had obvious "
        ...
        String pt3History = String.format("This patient is a 67 year old man who "
                + "complained of awareness of a rapid pulse rate and a forceful "
                + "heart beat.  In addition, he reported that his exercise tolerance had "
                + "decreased, and that he would get short of breath and fatigued more "
                + "easily than ever before. He had no previous history of heart "
                + "disease.  A previous physical examination done a year ago had discovered "
                + "a pulsatile abdominal mass, with an audible bruit in its vicinity.  He "
                + "had good femoral pulses bilaterally at that time.  Examination now "
                + "confirms the presence of the pulsatile mass and bruit.  The cardiac exam "
                + "reveals a rapid heart rate with a very forceful apical impulse.  There "
                + "was a grade 2/6 systolic murmur heard at the apex and over the second "
                + "intercostal space to the left of the sternum.  No diastolic murmurs were "
                + "heard.  Prominent capillary pulsations were noted in the nail beds and "
                + "under the tongue.  Femoral pulses were present bilaterally, athough the "
                + "left was weaker than the right.  The patient was taken to the catheterization "
        ...
        String pt4History = String.format("This patient is a 41 year old mother "
                + "of three children who was well (except for the usual colds, etc.) "
                + "until last year when she began to experience shortness of breath on "
                + "even minor exercise, and episodes of shortness of breath during the "
                + "night which were relieved by standing up.  She denied any chest "
                + "pain, chest infections, palpitations; but did report some mild "
                + "swelling of the ankles.  Her physical activities have become "
                + "severely limited because of the shortness of breath.  She has been "
                + "taking several medications at the request of her physician.  She "
                + "comes to you for hemodynamic evaluation and assessment.  Her exam "
                + "reveals an enlarged heart, regular rhythm, an S-3 gallop, no murmurs, "
                + "and fine rales at both lung bases.  There was 2+ ankle and pretibial edema.");
        ...
        String pt5History = String.format("The patient is a 60 year old man who "
                + "entered the hospital with severe crushing substernal chest "
                + "pain of two hours duration.  He appears short of breath.  He "
                + "is diaphoretic, has cold clammy skin, and his nail-beds are "
                + "cyanotic.  He is very lethargic and seems somewhat confused.  He "
                + "has low blood pressure, elevated jugular venous pressure, a "
                + "regular cardiac rhythm with an S-3 gallop, and examination of the "
                + "lungs reveals bibasilar fine rales.  He has no edema.  His "
                + "urine output seems to be very scanty.");
        ...
        String pt6History = String.format("This 55 year old male bank vice-president "
                + "had been in excellent health until one month ago when he began "
                + "to notice unusual shortness of breath and substernal pressure "
                + "which was associated with physical exercise.  The symptoms would "
                + "be brought on quite predictably after carrying a heavy briefcase "
                + "up to the third floor of the parking garage, for example.  Rest "
                + "would always relieve the discomfort after a few minutes.  He had "
                + "a known heart murmur which was first noted on a routine physical "
                + "examination ten years ago, but at the time it had been considered "
                + "to be an \"innocent\" murmur.  On the day of admission he had been "
                + "carrying a bag of cement to his backyard when he suddenly became "
                + "lightheaded and then lost conciousness.");
    ...
    private class DisplayPatientHistoryAction extends AbstractAction {
        ...
        public DisplayPatientHistoryAction(Patient patient) {
            ...
            putValue(Action.SHORT_DESCRIPTION, String.format("Display "
                    + pt.getName() + " History"));
        }
        ...
        public void actionPerformed(ActionEvent e) {
            ...
            System.out.println("Loading patient history for "
                    + pt.getName() + ".");
        }
    }
}
